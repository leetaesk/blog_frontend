import type { CommonResponseDto } from '@/types/CommonResponseDto';

export type TogglePostLikeRequestDto = {
  postId: number;
};

// 클라이언트에 응답할 결과 데이터 타입
export type TogglePostLikeResultType = {
  postId: number;
  liked: boolean;
  newLikesCount: number;
};

export type TogglePostLikeResponseDto = CommonResponseDto<TogglePostLikeResultType>;

export type ToggleCommentLikeRequestDto = {
  commentId: number;
};

// 클라이언트에 응답할 결과 데이터 타입
export type ToggleCommentLikeResultType = {
  commentId: number;
  liked: boolean;
  newLikesCount: number;
};

export type ToggleCommentLikeResponseDto = CommonResponseDto<ToggleCommentLikeResultType>;
